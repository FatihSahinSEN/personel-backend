...
class Sehirler extends ResourceController
{
    use ResponseTrait;
    ...
    public function index(){
        $data = $this->model->groupBy('sehir')->orderBy('sehir','asc')->findAll();
        if($data){
            $response = [
                "status" => true,
                "code" => 200,
    ...
    public function filter($filter = false){
        if($filter){
            $filter = '%'.$filter.'%';
            $data = $this->model->like('sehir',$filter)->groupBy('sehir')->orderBy('sehir','asc')->findAll();
            if($data){
                $response = [
                    "status" => true,
                    "code" => 200,
    ...
    {
       return $this->failNotFound('ERR_SEHIR_SILINEMEZ');
    }
