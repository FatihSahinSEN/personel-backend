...
class Uyruklar extends ResourceController
{
    use ResponseTrait;

    /**
     * @var \App\Models\Uyruklar
     */
    protected $model;
    /**
     * @var array[]
    ...
    {
        $this->model = new \App\Models\Uyruklar();
    }
    ...
    public function create()
    {
        ...
        $kontrol = $this->model->where('uyruk',$data["uyruk"])->first();
        if($kontrol){
            $response = [
                'status' => false,
                'code' => 200,
                'message' => 'ERR_UYRUK_MEVCUT'
            ];
            return $this->respond($response,$response["code"]);
        }
    ...
    public function delete($id = null)
    {
        ...
        if($data){
            ...
            $response = [
                'status'   => 200,
                'error'    => null,
                'message' => 'MSG_UYRUK_SILINDI'
            ];
            return $this->respondDeleted($response);
        }else{
            return $this->failNotFound('ERR_UYRUK_BULUNAMADI');
        }
    }
}